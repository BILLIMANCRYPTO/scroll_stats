...
# Значения, которые нужно адаптировать под ваше API
API_KEY = 'SE6CI5YJBUJJX39SK9S3B3KWNH8194P222'
BASE_URL = 'https://api.scrollscan.com/api'
EXCEL_FILE = 'stats.xlsx'
ETHER_PRICE = 3000  # Текущий курс эфира

# Словарь для хранения информации о взаимодействии с контрактами
...
def get_transactions(wallet_address):
    ...
    # Получаем баланс ETH для текущего кошелька
    eth_balance = get_eth_balance(wallet_address)

    return eth_balance, total_contracts, total_eth_volume, len(unique_contracts), len(activity_by_month), len(activity_by_week), len(activity_by_day), list(contract_interactions.values())
...
wb = Workbook()
ws = wb.active
ws.append(['Wallet'] + ['ETH Balance', 'Total TX', 'Total USD Volume', 'Unique Contracts', 'Active Months', 'Active Weeks', 'Active Days'])

progress_bar = tqdm(total=len(wallet_addresses), desc='Processing wallets', position=0, leave=True)

for wallet_address in wallet_addresses:
    try:
        eth_balance, total_tx, total_eth_volume, unique_contracts, active_months, active_weeks, active_days, contract_counts = get_transactions(wallet_address)
        row_data = [wallet_address, eth_balance, total_tx, total_eth_volume * ETHER_PRICE, unique_contracts, active_months, active_weeks, active_days]
        ws.append(row_data)
